...
const LinkyAvatar = ({ size = 32, onPress }: { size?: number; onPress?: () => void }) => (
  <TouchableOpacity onPress={onPress} activeOpacity={0.7} style={{ width: size, height: size, borderRadius: size / 3, backgroundColor: '#22C55E', alignItems: 'center', justifyContent: 'center' }}>
    <Text style={{ fontSize: size * 0.4, fontWeight: '900', color: '#FFF', letterSpacing: 1 }}>AI</Text>
  </TouchableOpacity>
);
    ...
    <View style={styles.dashContainer}>
      <View style={styles.dashAvatarRow}>
        ...
        <View>
          ...
          <Text style={styles.dashSubtitle}>Your AI networking assistant</Text>
        </View>
      </View>
      ...
      {suggestions.map((s) => (
        <TouchableOpacity key={s} style={[styles.dashChip, { backgroundColor: isDark ? 'rgba(34,197,94,0.1)' : 'rgba(34,197,94,0.08)' }]} onPress={() => onSuggestion(s)} activeOpacity={0.7}>
          <View style={styles.dashChipDot} />
          <Text style={styles.dashChipText}>{s}</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
...
const ProOnlyScreen = ({ onUpgrade }: { onUpgrade: () => void }) => (
  <View style={styles.proContainer}>
    <LinkyAvatar size={28} />
    <Text style={styles.proTitle}>Linky AI</Text>
    <Text style={styles.proSub}>Your AI networking assistant</Text>
    <View style={styles.proDivider} />
    <Lock size={32} color="#999" />
    <Text style={styles.proLocked}>Linky AI is a LINKUP PLUS feature</Text>
      ...
      <TouchableOpacity style={styles.profileCard} onPress={onPress} activeOpacity={0.7}>
        ...
        <View style={styles.profileInfo}>
          ...
          {profile.occupation || profile.company ? (
            <Text style={styles.profileRole} numberOfLines={1}>{[profile.occupation, profile.company].filter(Boolean).join(' at ')}</Text>
          ) : null}
          {profile.skills?.length > 0 && (
            <Text style={styles.profileSkills} numberOfLines={1}>{profile.skills.slice(0, 3).join(' Â· ')}</Text>
          )}
          {profile.city || profile.country ? (
            <Text style={styles.profileLocation} numberOfLines={1}>{[profile.city, profile.country].filter(Boolean).join(', ')}</Text>
          ) : null}
        </View>
      </TouchableOpacity>
      {onConnect && (
...
const TypingIndicator = ({ seconds }: { seconds: number }) => (
  <View style={styles.typingRow}>
    <LinkyAvatar size={28} />
    <View style={styles.typingBubble}>
      <Text style={styles.typingLabel}>Thinking{seconds > 0 ? ` ${seconds}s` : '...'}</Text>
    </View>
  </View>
);
...
export default function LinkyScreen({ navigation }: any) {
  ...
  const handleConnect = async (target: MiniProfile) => {
    ...
    try {
      ...
      Alert.alert('Connection failed', em);
    } finally {
  ...
  return (
    <View style={[styles.root, appBackground(isDark)]}>
      <SafeAreaView edges={['top']} style={[styles.header, { backgroundColor: isDark ? COLORS.darkBgSec : COLORS.lightBgSec, borderBottomColor: isDark ? 'rgba(34,197,94,0.15)' : 'rgba(34,197,94,0.1)' }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <View>
            <View style={styles.headerNameRow}>
              ...
              <CheckCircle size={14} color="#3897F0" fill="#3897F0" />
            </View>
            <View style={styles.headerMetaRow}>
              <View style={styles.headerOnlineDot} />
              <Text style={styles.headerMetaText}>Online · AI Assistant</Text>
            </View>
          </View>
        </View>
        <TouchableOpacity style={styles.headerBtn} onPress={() => {
      ...
      {!isPro ? (
        ...
        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0}>
          <FlatList
            ...
            ListFooterComponent={loading ? <TypingIndicator seconds={thinkingSec} /> : null}
          />
              ...
              <View style={[styles.draftBanner, { backgroundColor: isDark ? COLORS.darkBgSec : '#F0F0F0' }]}>
                <View style={styles.draftRow}>
                  ...
                  <Text style={[styles.draftTo, { color: textColor(isDark) }]} numberOfLines={1}>
                    ...
                    {draftIntro.recipient.occupation ? ` Â· ${draftIntro.recipient.occupation}` : ''}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  profileSkills: { fontSize: 10, color: COLORS.primary, fontWeight: '700', marginTop: 1 },
