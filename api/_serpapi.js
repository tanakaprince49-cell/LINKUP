...
export async function canSearch({ plus = false, uid = '' } = {}) {
  ...
  const [acct, led] = await Promise.all([accountCredits(), ledger()]);
  ...
  const pages = Math.max(0, Math.min(perRun, hourLeft));
  if (!pages) {
...
export function prefilter(organic, { need = '', roles = [], words = [] } = {}) {
  ...
  for (const r of organic || []) {
    ...
    if (/\b(news|newsletter?\s*paper|jobs?\s*at|salary|download|sign ?up|log ?in|privacy|cookie)\b/i.test(hay)) continue;
...
async function cleanLink(rawLink) {
  ...
  try {
    const res = await fetch(`https://www.google.com${l}`, {
...
export async function findLeads(need, { place = '', plus = false, uid = '', askedBy = '' } = {}) {
  ...
  await led.ref.set({
    ...
    hourUsed: led.hour + searches, updatedAt: started, lastUid: uid, lastQuery: primary,
