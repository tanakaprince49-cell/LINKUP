...
export type CompactBuilderProfile = {
  uid: string;
  displayName?: string;
  role?: string;
  goals?: string[] | string;
  ...
  availability?: string;
  personality?: string;
  roleSignals?: Record<string, unknown>;
};
...
export function getGeminiKey() {
  return String(process.env.GEMINI_API_KEY || process.env.EXPO_PUBLIC_GEMINI_API_KEY || process.env.GOOGLE_API_KEY || '').trim();
}

export function sendError(res: any, status: number, message: string, technical?: unknown) {
  setCors(res);
  res.status(status).json({
    error: message,
    technical: String(technical || message).replace(getGeminiKey(), '[redacted-key]').slice(0, 500),
  });
}
...
export function compactProfile(profile: any): CompactBuilderProfile {
  return {
    ...
    goals: Array.isArray(profile?.goals || profile?.lookingFor)
      ? (profile?.goals || profile?.lookingFor).slice(0, 5).map((entry: unknown) => String(entry).slice(0, 80))
      : String(profile?.goals || '').slice(0, 160),
    skills: Array.isArray(profile?.skills) ? profile.skills.slice(0, 8).map((entry: unknown) => String(entry).slice(0, 80)) : [],
    industries: Array.isArray(profile?.industries) ? profile.industries.slice(0, 6).map((entry: unknown) => String(entry).slice(0, 80)) : [],
...
export function clippedJson(value: unknown, max = MAX_PROFILE_CHARS) {
  return JSON.stringify(value ?? {}).slice(0, max);
}

export async function geminiText(prompt: string, options: { temperature?: number; maxOutputTokens?: number; responseMimeType?: string } = {}) {
...
export function localRank(me: CompactBuilderProfile, candidates: CompactBuilderProfile[], maxCandidates: number) {
  ...
  return candidates
    .map((candidate) => {
      ...
      const complementary =
        ...
        myRole !== String(candidate.role || '').toLowerCase()
          ? 8
          : 0;
